...
async function load() {
  ...
  container.innerHTML = `
    <div class="item-meta">
      <a href="posts.html?seccao=${encodeURIComponent(post.sections?.slug ?? '')}" class="board-tag">/${esc(post.sections?.slug ?? '')}/</a> ·
      Anónimo <span class="post-no">ID:${pseudoId(slug)} No.${pseudoNo(slug)}</span> ·
